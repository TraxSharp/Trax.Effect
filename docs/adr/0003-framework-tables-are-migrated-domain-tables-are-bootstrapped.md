...
- **Trax's own tables** (the `trax` schema) are created only by the shipped migrations
  ([0001](./0001-schema-changes-are-hand-written-sql.md)). `EnsureCreated` is never used for
  them.
...
- `MigrationSchemaTests` builds the state machine tables from the **shipped** migrations
  ...
  exist. It needs a live Postgres and is skipped where there is none.

Not covered: only the state machine tables have this round-trip. Every other Trax table is
covered by the naming and sequence checks alone, so model-versus-DDL drift elsewhere is
caught at runtime rather than by a test.
