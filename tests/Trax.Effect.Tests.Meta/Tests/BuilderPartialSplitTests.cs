...
public class BuilderPartialSplitTests
{
    ...
    /// <summary>
    /// Builder directories whose Build() method living in the main state file is grandfathered in.
    /// New builders must follow the split.
    /// </summary>
        ...
        found
            ...
            .BeEmpty(
                $"the Build() method for '{builderName}' must live in '{dotBuildFile}', not in another "
                    ...
                    + "so build-time validation lives in a dedicated file. Found Build() in:\n  "
                    + string.Join("\n  ", found)
            );
    }
}
